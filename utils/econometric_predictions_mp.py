...
rom pmdarima.arima import StepwiseContext
import pandas as pd
import numpy as np
...
def make_predictions(this_series):
    ...
    column_names=arima_predictions.objects.values('series_title').annotate(cnt=Count('id')).filter(cnt=12).values_list('series_title', flat=True).distinct().exclude(series_title=this_series)
...
if __name__=='__main__':
    ...
    selections_list=arima_predictions.objects.values('series_title').annotate(cnt=Count('id')).filter(cnt=12).values_list('series_title', flat=True).distinct()[:10]
    p=Pool(4)
