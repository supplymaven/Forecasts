...
def make_predictions(this_series):
    ...
    pvalues=[pv for pv in model_dictionary['pvalues']][:-1] # the last one is actually the variance
    coefficients=[c for c in model_dictionary['params']][:-1] # the last one is actually the variance
    ...
    for j in range(len(predictions)): 
        sql='insert into machinelearning_arima_predictions (future_date,inx,forecasted_series_id) values ("' + str(future_dates[j]) + '",' + str(predictions[j]) + ',' + str(forecast) + ');'
        cursor.execute(sql)
        db.commit()
