...
def home(request):
    ...
    selections_list=series_names.objects.filter(id__in=econometric_forecast.objects.values_list('series', flat=True).distinct()).values_list('series_title',flat=True).distinct().exclude(series_title__in=
    if request.POST:
